...
import json
from PySide6.QtCore import Qt, QMimeData, Signal, QRect, QPoint, QSize
...
class MasonryQuestionCard(QLabel):
    """
    Pinterest-style question card with variable height.
    
    Design features:
    - Clean white card with shadow effect
    - Full question text (no truncation unless very long)
    ...
    clicked = Signal(dict)
    right_clicked = Signal(dict, object)  # question_data, global_pos
    
    def __init__(self, question_data: dict, tag_colors: dict = None, parent=None):
        super().__init__(parent)
        self.question_data = question_data
        self.tag_colors = tag_colors or {}
        self.is_selected = False
        ...
        # Card styling - white with border
        self.setStyleSheet("""
            QLabel {
                background-color: #ffffff;
        ...
        # No max height - let content determine it
    
    def _build_card(self):
        """Build Pinterest-style card HTML."""
        q = self.question_data
        ...
        # Smart truncation - show more text but not too much
        preview = self._smart_truncate(question_text, max_chars=300)
        ...
        if tags:
            ...
            for tag in tags[:4]:  # Show up to 4 tags
                ...
                tag_chips.append(
                    ...
                    f'padding: 4px 10px; border-radius: 12px; font-size: 11px; '
                    f'font-weight: 600; margin: 2px 4px 2px 0;">{tag}</span>'
                )
            tag_html = '<div style="margin: 12px 0 8px 0;">' + "".join(tag_chips) + '</div>'
        
        # Build card HTML
        html = f"""
        <div style="line-height: 1.5;">
            <div style="margin-bottom: 12px;">
                <span style="color: #2563eb; font-size: 18px; font-weight: bold;">Q{qno}</span>
                <span style="color: #9ca3af; font-size: 12px; margin-left: 10px;">📄 Pg {page}</span>
            </div>
            <div style="color: #1f2937; font-size: 14px; margin-bottom: 12px; line-height: 1.6;">
                {preview}
            </div>
            {tag_html}
            <div style="border-top: 1px solid #e5e7eb; padding-top: 10px; margin-top: 10px;">
                <div style="font-size: 12px; color: #6b7280; margin-bottom: 4px;">
                    <span style="font-weight: 600;">📖 {question_set}</span>
                </div>
    ...
    def _smart_truncate(self, text: str, max_chars: int = 300) -> str:
        ...
        if boundary > max_chars * 0.6:  # If we found a good boundary
    ...
    def _get_tag_color(self, tag: str) -> str:
        """Get color for tag."""
        return self.tag_colors.get(tag, self.tag_colors.get(tag.lower(), "#6b7280"))
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            self.right_clicked.emit(self.question_data, event.globalPos())
        super().mousePressEvent(event)
...
class MasonryLayout(QLayout):
    """
    Custom layout that arranges items in columns like Pinterest.
    
    Items flow into the shortest column, creating a balanced masonry effect.
    """
    
    def __init__(self, parent=None, column_count=3, spacing=12):
        super().__init__(parent)
        self.item_list = []
    ...
    def setGeometry(self, rect):
        ...
        column_width = (available_width - (self.column_count - 1) * self._spacing) // self.column_count
        ...
        for item in self.item_list:
            ...
            # Calculate position
            x = rect.x() + col_index * (column_width + self._spacing)
            y = rect.y() + column_heights[col_index]
            
            # Set widget size
            widget.setFixedWidth(column_width)
            
            # Get natural height based on width
            widget.adjustSize()
            ...
            column_heights[col_index] += height + self._spacing
    ...
    def sizeHint(self):
        ...
        # Estimate height based on average item height
        total_height = sum(item.widget().sizeHint().height() for item in self.item_list if item.widget())
        avg_height = total_height / len(self.item_list) if self.item_list else 200
        estimated_rows = (len(self.item_list) + self.column_count - 1) // self.column_count
        
        return QSize(800, int(avg_height * estimated_rows))
...
class QuestionMasonryView(QScrollArea):
    """
    Pinterest-style masonry grid for questions.
    
    Features:
    - Multi-column grid (3 columns default)
    ...
    def __init__(self, parent=None, columns=3):
        ...
        # Container with masonry layout
        self.container = QWidget()
        self.masonry_layout = MasonryLayout(self.container, column_count=columns, spacing=12)
        self.container.setLayout(self.masonry_layout)
        
        self.setWidget(self.container)
        
        # Track cards and selection
        self.cards = []
    ...
    def add_questions(self, questions: list[dict], tag_colors: dict = None):
        """
        Add questions as masonry cards.
        
        Args:
            questions: List of question data dicts
            tag_colors: Dict mapping tag names to colors
        """
        for question in questions:
            card = MasonryQuestionCard(question, tag_colors, self)
            card.clicked.connect(self._on_card_clicked)
            card.right_clicked.connect(self._on_card_right_clicked)
            
            self.masonry_layout.addItem(QWidgetItem(card))
    ...
    def _on_card_clicked(self, question_data: dict):
        ...
        # Select clicked card
        sender = self.sender()
        if isinstance(sender, MasonryQuestionCard):
    ...
    def _on_card_right_clicked(self, question_data: dict, global_pos):
        """Handle right-click on card."""
        # Show context menu if parent supports it
        if hasattr(self.parent(), 'show_question_context_menu'):
            self.parent().show_question_context_menu(question_data, global_pos)
    
    def set_column_count(self, count: int):
        """Change number of columns."""
        self.column_count = count
        self.masonry_layout.column_count = count
        self.container.updateGeometry()
